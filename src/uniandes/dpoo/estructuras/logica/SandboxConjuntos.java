...
public class SandboxConjuntos
{
    ...
    {
        Collection<String> siguientes = new TreeSet<>();

        for (String palabra : arbolCadenas) {
            if (palabra.compareTo(cadena) >= 0) {
                siguientes.add(palabra);
            }
        }
    ...
    public void eliminarCadenaSinMayusculasOMinusculas( String cadena )
    {
        if (arbolCadenas != null) {
            ...
            TreeSet <String> arbolMinusculas = new TreeSet<String>( );
            for (String s : arbolCadenas) {
                arbolMinusculas.add(s.toLowerCase());
            }
            
            if (arbolMinusculas.contains(palabra)) {
